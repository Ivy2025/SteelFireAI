...
# 模型路径配置
MODEL_PATHS = {
    "fy_reduction": Path("results/models/rf_random_fy_reduction.pkl"),
    "fu_reduction": Path("results/models/rf_random_fu_reduction.pkl"),
    "E_reduction": Path("results/models/rf_random_E_reduction.pkl"),
}
...
def load_data():
    ...
    df = df.replace(r'^\s*$', np.nan, regex=True)  # 替换空字符串和空格为NaN
    return df
