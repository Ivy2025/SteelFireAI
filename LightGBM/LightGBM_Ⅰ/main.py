...
from train_lightgbm import make_group_split_for_target, train_one_target_one_plan


VALID_PLANS = ["plan1", "plan2"]
...
def main() -> None:
    ...
    # Same outer split for both plans under the same target.
    for target in targets:
        split_by_target[target] = make_group_split_for_target(
            data_df=df,
            target=target,
            test_size=0.2,
    ...
    for plan in plans:
        ...
        for target in targets:
            train_groups, test_groups = split_by_target[target]
    ...
    if set(plans) == set(VALID_PLANS):
        save_comparison(result_dir=args.result_dir, output_prefix=args.output_prefix)
    else:
        print("[INFO] Comparison file skipped because both plan1 and plan2 were not run together.")
