...
from src.cv import generate_group_folds
...
def _split_train_test_by_group(
	groups: pd.Series,
	test_size: float,
	random_state: int,
	...
	if len(unique_groups) < 2:
		raise ValueError("At least 2 unique groups are required for train/test split.")
	...
	if len(train_idx) == 0 or len(test_idx) == 0:
		raise ValueError("Failed to create non-empty train/test split by group.")
...
def run_training(settings: Settings) -> None:
	...
	if settings.use_test_holdout:
		train_idx, test_idx = _split_train_test_by_group(
			groups=groups,
		...
		logger.info(
			"Holdout split created by group. train_rows=%d test_rows=%d train_groups=%d test_groups=%d",
			len(train_idx),
			len(test_idx),
	...
	outer_splits = min(settings.n_splits, int(groups_train.nunique()))
	if outer_splits < 2:
		raise ValueError("At least 2 unique groups are required for GroupKFold CV.")
	...
	for target_name in target_cols:
		...
		for fold_idx, fold_train_idx, val_idx in generate_group_folds(x_train_all, groups_train, outer_splits):
			x_train = x_train_all.iloc[fold_train_idx]
			y_train = y_train_target.iloc[fold_train_idx]
			g_train = groups_train.iloc[fold_train_idx]
